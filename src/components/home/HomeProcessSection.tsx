...
const processFallbacks = [
  {
    title: 'Client Brief',
    description: 'We define the requirement, application, constraints, and delivery target before production begins.',
  },
  {
    title: 'Mechanical CAD Design',
    description: 'Our engineers translate the brief into accurate, production-ready drawings and assemblies.',
  },
  {
    title: 'Laser Cutting & Machining',
    description: 'Components are cut and machined to controlled tolerances using the right production process.',
  },
  {
    title: 'Welding & Erection',
    description: 'Parts are aligned, welded, and assembled into a strong, dependable finished structure.',
  },
  {
    title: 'Surface Painting & Finishings',
    description: 'Protective coatings and final finishes are applied for durability, safety, and presentation.',
  },
  {
    title: 'Final Delivery',
    description: 'The completed product is inspected, documented, and prepared for safe delivery to site.',
  },
]
...
export function HomeProcessSection({ block }: { block: HomeProcessLayoutBlock }) {
  ...
  const steps = useMemo(
    () => processFallbacks.map((fallback, index) => ({ ...fallback, title: block.steps[index]?.title || fallback.title })),
    [block.steps],
  )
  ...
  return (
    ...
    >
      <header className="process-header" data-grid-alignment="process-columns">
        ...
        <div className="process-mark" data-position="top-right" aria-hidden="true"><span>SW</span></div>
      </header>
      ...
      <div className="process-grid">
        ...
        >
          {steps
            .slice(0, 3)
            .map((step, index) => renderStep(step, index, leftColumnHasActiveStep))}
        </div>
        <div className="process-model-stage" data-testid="process-model-stage" aria-label="Rotating product wireframe">
          <HomeProcessModel />
        </div>
        <div
